...
export default {
    install(app) {
        ...
        app.directive('alert', {
            mounted(el, b) {
                ...
                if (b.modifiers && b.modifiers.closeable) {
                    ...
                    el.innerHTML += '  <button type="button"  class="btn-close" data-dismiss="alert" aria-label="Close"></button>'
                }
            }
        })
        ...
        // Helpers
        // Login Dialog
        app.config.globalProperties.$login_dialog = (login, register, email, reset, cancel) => app.config.globalProperties.$dialog(
            {
                width: "20em",
                btn_center: true,
                title: "Login",
            ...
            (a, f) => a === 'default' ? login(f) : a === 'close' ? cancel() : null
        );
    }
}