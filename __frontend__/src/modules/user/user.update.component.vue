...
import user_types from "./user.store.types";
import {
  invalid_first_name,
  invalid_last_name,
...
export default {
  computed: {
    ...mapGetters({
      current_user: user_types.getters.get_current_user,
    }),
    photo: (t) =>
  ...
  created() {
    this.$store.dispatch(user_types.actions.getCurrentUser);
  },
  ...
  methods: {
    ...
    submitFile() {
      ...
      this.$store
        .dispatch(user_types.actions.updateUser, formData)
        .then(() => this.onSuccess())
        .catch(() => this.onError());
    },
