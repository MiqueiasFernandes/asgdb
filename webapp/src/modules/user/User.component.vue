<template>
  <div>
    <Display ico="people">
      User
      <router-link :to="'/user/new/edit'">
        <Button success ico="plus" class="float-right"> New</Button>
      </router-link>
    </Display>
    <div v-if="page">
      <Display lead>
        Has {{ actives }} active in <strong>{{ page.count }} users</strong>.</Display
      >
      <table
      ...
      >
        ...
        <thead class="table-light">
          <tr>
            ...
            <th class="text-center">
              <Button color="none" sm
                >Active <Icon name="arrow-up" sm
              /></Button>
            </th>
            <th>
        ...
        <tbody>
          <tr v-for="(user, index) in page.results" :key="user.id">
            ...
            <td>{{ user.registered_at.replaceAll('.', '/').slice(0, -4) + user.registered_at.slice(-2) }}</td>
            <td class="text-center">
              <Icon
...
export default {
  ...
  computed: {
    current_page: (t) =>
      parseInt(t.$route.query.page ? t.$route.query.page : t.page ? t.page.page : 1),
  },
  data: () => ({
    page: null,
    loading: false,
  ...
  methods: {
    ...
    view(id) {
      this.$router.push({ path: `/user/${id}/view` });
    },
    edit(id) {
